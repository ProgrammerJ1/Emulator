...
pub fn bit_setting(rng: &mut ThreadRng) {
    ...
    {
        let old_number_strings:[String; 3]=[format!("{:<64b}",numbers[0]),format!("{:<64b}",numbers[1].to_string()),format!("{:<64b}",numbers[2].to_string())];
    ...
    for i in 0..3 {
        assert_eq!(format!("{:<64b}",numbers[i]),new_number_strings[i]);
    }
}
